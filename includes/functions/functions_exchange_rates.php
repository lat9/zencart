...
function zen_update_currencies($cli_Output = false)
{
    ...
    foreach ($currency as $next_currency) {
        $server_used = CURRENCY_SERVER_PRIMARY;
        $rate = '';
        $quote_function = 'quote_' . CURRENCY_SERVER_PRIMARY . '_currency';
        if (function_exists($quote_function)) {
            $rate = $quote_function($next_currency['code']);
        }
        ...
        if (empty($rate) && !empty(CURRENCY_SERVER_BACKUP)) {
            ...
            $msg = sprintf(WARNING_PRIMARY_SERVER_FAILED, CURRENCY_SERVER_PRIMARY, $next_currency['title'], $next_currency['code']);
        ...
        if (!empty($rate)) {
            ...
            $zco_notifier->notify('ADMIN_CURRENCY_EXCHANGE_RATE_MULTIPLIER', $next_currency['code'], $multiplier, $rate);
            ...
            if ($next_currency['decimal_places'] === '0') {
                $rate = (int)$rate;
            }

            if (!empty($rate)) {
                $zco_notifier->notify('ADMIN_CURRENCY_EXCHANGE_RATE_SINGLE', $next_currency['code'], $rate);
                $db->Execute(
                    "UPDATE " . TABLE_CURRENCIES . "
                        SET value = '" . round((float)$rate, 8) . "',
                            last_updated = now()
                      WHERE currencies_id = " . $next_currency['currencies_id'] . "
                      LIMIT 1"
                );
                $msg = sprintf(TEXT_INFO_CURRENCY_UPDATED, $next_currency['title'], $next_currency['code'], round((float)$rate, 8), $server_used);
                if (is_object($messageStack)) {
                ...
                $msg = sprintf(ERROR_CURRENCY_INVALID, $next_currency['title'], $next_currency['code'], $server_used);
                if (is_object($messageStack)) {
...
function quote_boc_currency($currencyCode = '', $base = DEFAULT_CURRENCY)
{
    ...
    if (empty($BOCdata)) {
        ...
        if (empty($BOCdata) || empty($BOCdata['observations'])) {
            return false; // no data means unable to continue with updates
        }
    }
...
function doCurlCurrencyRequest($method, $url, $vars = '') {
    ...
    $curl_options = [
        ...
        CURLOPT_USERAGENT => empty($_SERVER['HTTP_USER_AGENT']) ? HTTP_CATALOG_SERVER . DIR_WS_CATALOG : $_SERVER['HTTP_USER_AGENT'],
        CURLOPT_REFERER => HTTP_CATALOG_SERVER . DIR_WS_CATALOG,
        CURLOPT_FOLLOWLOCATION => true,
        CURLOPT_RETURNTRANSFER => true,
    ];
    if (strtoupper($method) === 'POST' && $vars !== '') {
        $curl_options[CURLOPT_POST] = true;
        $curl_options[CURLOPT_POSTFIELDS] = $vars;
    }
